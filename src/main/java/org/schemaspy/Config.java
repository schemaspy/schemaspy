...
public final class Config implements HtmlConfig, GraphvizConfig {
    ...
    private Boolean noDbObjectPaging;
    private Integer dbObjectPageLength;
    private Boolean dbObjectLengthChange;

    // DataTables options for standard_table (Columns) table in src/main/resources/layout/tables/table.html
    private Boolean noTablePaging;
    private Integer tablePageLength;
    private Boolean tableLengthChange;

    // DataTables options for indexes_table (Indexes) table in src/main/resources/layout/tables/table.html
    private Boolean noIndexPaging;
    private Integer indexPageLength;
    private Boolean indexLengthChange;
    ...
    private Boolean noCheckPaging;
    private Integer checkPageLength;
    private Boolean checkLengthChange;

    // DataTables options for routine_table (Routines) table in src/main/resources/layout/routines.html
    private Boolean noRoutinePaging;
    private Integer routinePageLength;
    private Boolean routineLengthChange;

    // DataTables options for fk_table (Constraints) table in src/main/resources/layout/constraint.html
    private Boolean noFkPaging;
    private Integer fkPageLength;
    private Boolean fkLengthChange;

    // DataTables options for column_table (Columns) table in src/main/resources/layout/column.html
    private Boolean noColumnPaging;
    private Integer columnPageLength;
    private Boolean columnLengthChange;

    // DataTables options for all tables in src/main/resources/layout/anomalies.html
    private Boolean noAnomaliesPaging;
    private Integer anomaliesPageLength;
    private Boolean anomaliesLengthChange;
    ...
    public boolean isNoDbObjectPaging() {
        if (noDbObjectPaging == null) {
            noDbObjectPaging = options.remove("-noDbObjectPaging");
        }
        return noDbObjectPaging;
    }

    public int getDbObjectPageLength() {
        if (dbObjectPageLength == null) {
            String dbObjectPageLengthString = pullParam("-dbObjectPageLength");
            if (hasText(dbObjectPageLengthString)) {
                try {
                    dbObjectPageLength = Integer.valueOf(dbObjectPageLengthString);
                } catch (NumberFormatException numberFormatException) {
                    LOGGER.warn(numberFormatException.getMessage(), numberFormatException);
                }
            }
            else {
                dbObjectPageLength = 50;
            }
        }
    ...
    public boolean isDbObjectLengthChange() {
        if (dbObjectLengthChange == null) {
            dbObjectLengthChange = options.remove("-dbObjectLengthChange");
        }
    ...
    public boolean isNoTablePaging() {
        if (noTablePaging == null) {
            noTablePaging = options.remove("-noTablePaging");
        }
        return noTablePaging;
    }

    public int getTablePageLength() {
        if (tablePageLength == null) {
            String tablePageLengthString = pullParam("-tablePageLength");
            if (hasText(tablePageLengthString)) {
                try {
                    tablePageLength = Integer.valueOf(tablePageLengthString);
                } catch (NumberFormatException numberFormatException) {
                    LOGGER.warn(numberFormatException.getMessage(), numberFormatException);
                }
            }
    ...
    public boolean isTableLengthChange() {
        if (tableLengthChange == null) {
            tableLengthChange = options.remove("-tableLengthChange");
        }
    ...
    public boolean isNoIndexPaging() {
        if (noIndexPaging == null) {
            noIndexPaging = options.remove("-noIndexPaging");
        }
        return noIndexPaging;
    }

    public int getIndexPageLength() {
        if (indexPageLength == null) {
            String indexPageLengthString = pullParam("-indexPageLength");
            if (hasText(indexPageLengthString)) {
                try {
                    indexPageLength = Integer.valueOf(indexPageLengthString);
                } catch (NumberFormatException numberFormatException) {
                    LOGGER.warn(numberFormatException.getMessage(), numberFormatException);
                }
            }
    ...
    public boolean isIndexLengthChange() {
        if (indexLengthChange == null) {
            indexLengthChange = options.remove("-indexLengthChange");
        }
    ...
    public boolean isNoCheckPaging() {
        if (noCheckPaging == null) {
            noCheckPaging = options.remove("-noCheckPaging");
        }
        return noCheckPaging;
    }

    public int getCheckPageLength() {
        if (checkPageLength == null) {
            String checkPageLengthString = pullParam("-checkPageLength");
            if (hasText(checkPageLengthString)) {
                try {
                    checkPageLength = Integer.valueOf(checkPageLengthString);
                } catch (NumberFormatException numberFormatException) {
                    LOGGER.warn(numberFormatException.getMessage(), numberFormatException);
                }
            }
    ...
    public boolean isCheckLengthChange() {
        if (checkLengthChange == null) {
            checkLengthChange = options.remove("-checkLengthChange");
        }
    ...
    public boolean isNoRoutinePaging() {
        if (noRoutinePaging == null) {
            noRoutinePaging = options.remove("-noRoutinePaging");
        }
        return noRoutinePaging;
    }

    public int getRoutinePageLength() {
        if (routinePageLength == null) {
            String routinePageLengthString = pullParam("-routinePageLength");
            if (hasText(routinePageLengthString)) {
                try {
                    routinePageLength = Integer.valueOf(routinePageLengthString);
                } catch (NumberFormatException numberFormatException) {
                    LOGGER.warn(numberFormatException.getMessage(), numberFormatException);
                }
            }
            else {
                routinePageLength = 50;
            }
        }
    ...
    public boolean isRoutineLengthChange() {
        if (routineLengthChange == null) {
            routineLengthChange = options.remove("-routineLengthChange");
        }
    ...
    public boolean isNoFkPaging() {
        if (noFkPaging == null) {
            noFkPaging = options.remove("-noFkPaging");
        }
        return noFkPaging;
    }

    public int getFkPageLength() {
        if (fkPageLength == null) {
            String fkPageLengthString = pullParam("-fkPageLength");
            if (hasText(fkPageLengthString)) {
                try {
                    fkPageLength = Integer.valueOf(fkPageLengthString);
                } catch (NumberFormatException numberFormatException) {
                    LOGGER.warn(numberFormatException.getMessage(), numberFormatException);
                }
            }
    ...
    public boolean isFkLengthChange() {
        if (fkLengthChange == null) {
            fkLengthChange = options.remove("-fkLengthChange");
        }
    ...
    public boolean isNoColumnPaging() {
        if (noColumnPaging == null) {
            noColumnPaging = options.remove("-noColumnPaging");
        }
        return noColumnPaging;
    }

    public int getColumnPageLength() {
        if (columnPageLength == null) {
            String columnPageLengthString = pullParam("-columnPageLength");
            if (hasText(columnPageLengthString)) {
                try {
                    columnPageLength = Integer.valueOf(columnPageLengthString);
                } catch (NumberFormatException numberFormatException) {
                    LOGGER.warn(numberFormatException.getMessage(), numberFormatException);
                }
            }
    ...
    public boolean isColumnLengthChange() {
        if (columnLengthChange == null) {
            columnLengthChange = options.remove("-columnLengthChange");
        }
    ...
    public boolean isNoAnomaliesPaging() {
        if (noAnomaliesPaging == null) {
            noAnomaliesPaging = options.remove("-noAnomaliesPaging");
        }
        return noAnomaliesPaging;
    }

    public int getAnomaliesPageLength() {
        if (anomaliesPageLength == null) {
            String anomaliesPageLengthString = pullParam("-anomaliesPageLength");
            if (hasText(anomaliesPageLengthString)) {
                try {
                    anomaliesPageLength = Integer.valueOf(anomaliesPageLengthString);
                } catch (NumberFormatException numberFormatException) {
                    LOGGER.warn(numberFormatException.getMessage(), numberFormatException);
                }
            }
            else {
                anomaliesPageLength = 10;
            }
        }
    ...
    public boolean isAnomaliesLengthChange() {
        if (anomaliesLengthChange == null) {
            anomaliesLengthChange = options.remove("-anomaliesLengthChange");
        }
