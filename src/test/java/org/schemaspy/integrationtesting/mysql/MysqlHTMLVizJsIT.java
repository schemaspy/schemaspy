...
public class MysqlHTMLVizJsIT {
	...
	private static URL expectedXML = MysqlHTMLVizJsIT.class
			.getResource("/integrationTesting/mysql/expecting/mysqlhtml_vizjs_svg/htmlit.htmlit.xml");
	private static URL expectedDeletionOrder = MysqlHTMLVizJsIT.class
			.getResource("/integrationTesting/mysql/expecting/mysqlhtml_vizjs_svg/deletionOrder.txt");
	private static URL expectedInsertionOrder = MysqlHTMLVizJsIT.class
			.getResource("/integrationTesting/mysql/expecting/mysqlhtml_vizjs_svg/insertionOrder.txt");
	...
	public synchronized void generateHTML() throws Exception {
		if (shouldRun.get()) {
			String[] args = new String[] { "-t", "mysql", "-db", "htmlit", "-s", "htmlit", "-host",
					...
					jdbcContainerRule.getContainer().getPassword(), "-o", outputPath.toString(), "-vizjs", "-connprops",
					"useSSL\\=false" };
	...
	public void producesSameContent() throws IOException {
		SoftAssertions softAssertions = HtmlOutputValidator.hasProducedValidOutput(outputPath, Paths.get("src", "test",
				"resources", "integrationTesting", "mysql", "expecting", "mysqlhtml_vizjs_svg"));
