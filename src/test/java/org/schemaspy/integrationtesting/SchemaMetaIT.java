...
public class SchemaMetaIT {
	...
	public void setup() throws IOException, SQLException {
		...
		dbmsMeta = serviceFixture.sqlService().getDbmsMeta();
	...
	public void commentsNullTableComment() throws Exception {
		...
		SchemaMeta schemaMeta = new SchemaMeta(
				"src/test/resources/integrationTesting/schemaMetaIT/input/nullTableComment.xml", "SchemaMetaIT",
				schema);
		Database databaseWithSchemaMeta = new Database(dbmsMeta, "SchemaMetaIT", catalog, schema);
		serviceFixture.databaseService().gatherSchemaDetails(config, databaseWithSchemaMeta, schemaMeta,
				progressListener);
	...
	public void commentsAreReplacedWithReplaceComments() throws Exception {
		...
		serviceFixture.databaseService().gatherSchemaDetails(config, database, schemaMeta, progressListener);
	...
	public void disableDiagramAssociations() throws Exception {
		...
		try (PrintWriter printWriter = new PrintWriter(withoutSchemaMetaOutput)) {
			dotFormatter.writeTableAllRelationships(database.getTablesMap().get("COMPANY"), false,
					new WriteStats(database.getTables()), printWriter);
		}
		...
		try (PrintWriter printWriter = new PrintWriter(withSchemaMetaOutput)) {
			dotFormatter.writeTableAllRelationships(databaseWithSchemaMeta.getTablesMap().get("COMPANY"), false,
					new WriteStats(databaseWithSchemaMeta.getTables()), printWriter);
		}
