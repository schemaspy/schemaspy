...
public class SchemaMetaIT {
	...
	@ClassRule
	public static H2MemoryRule h2MemoryRule = new H2MemoryRule("SchemaMetaIT")
	...
	public void setup() throws IOException, SQLException {
		...
		String[] args = { "-t", "src/test/resources/integrationTesting/dbTypes/h2memory", "-db", "SchemaMetaIT", "-s",
				"SCHEMAMETAIT", "-o", "target/integrationtesting/schemaMetaIT", "-u", "sa" };
	...
	public void commentsNullTableComment() throws Exception {
		Database database = new Database(dbmsMeta, "DatabaseServiceIT", catalog, schema);
		new DatabaseServiceFactory(sqlService).simple(config).gatherSchemaDetails(database, null, progressListener);

		SchemaMeta schemaMeta = new SchemaMeta(
				"src/test/resources/integrationTesting/schemaMetaIT/input/nullTableComment.xml", "SchemaMetaIT", schema,
				false);
		Database databaseWithSchemaMeta = new Database(dbmsMeta, "SchemaMetaIT", catalog, schema);
		new DatabaseServiceFactory(sqlService).simple(config).gatherSchemaDetails(databaseWithSchemaMeta, schemaMeta,
				progressListener);
		...
		assertThat(database.getTablesMap().get("ACCOUNT").getColumn("name").getComments())
				.isEqualToIgnoringCase(BY_SCRIPT_COMMENT);
		...
		assertThat(databaseWithSchemaMeta.getTablesMap().get("ACCOUNT").getComments())
				.isEqualToIgnoringCase(BY_SCRIPT_COMMENT);
		assertThat(databaseWithSchemaMeta.getTablesMap().get("ACCOUNT").getColumn("name").getComments())
				.isEqualToIgnoringCase(BY_SCRIPT_COMMENT);
		assertThat(databaseWithSchemaMeta.getTablesMap().get("ACCOUNT").getColumn("accountId").getComments())
				.isEqualToIgnoringCase(BY_SCHEMA_META_COMMENT);
	}
	...
	public void commentsNoTableComment() throws SQLException {
		...
		SchemaMeta schemaMeta = new SchemaMeta(
				"src/test/resources/integrationTesting/schemaMetaIT/input/noTableComment.xml", "SchemaMetaIT", schema,
				false);
		Database databaseWithSchemaMeta = new Database(dbmsMeta, "SchemaMetaIT", catalog, schema);
		new DatabaseServiceFactory(sqlService).simple(config).gatherSchemaDetails(databaseWithSchemaMeta, schemaMeta,
				progressListener);
	...
	public void commentsAreReplacedWithReplaceComments() throws Exception {
		SchemaMeta schemaMeta = new SchemaMeta(
				"src/test/resources/integrationTesting/schemaMetaIT/input/replaceComments.xml", "SchemaMetaIT", schema,
				false);
		Database database = new Database(dbmsMeta, "SchemaMetaIT", catalog, schema);
		new DatabaseServiceFactory(sqlService).simple(config).gatherSchemaDetails(database, schemaMeta,
				progressListener);
	...
	public void remoteTable() throws Exception {
		...
		SchemaMeta schemaMeta = new SchemaMeta(
				"src/test/resources/integrationTesting/schemaMetaIT/input/remoteTable.xml", "SchemaMetaIT", schema,
				false);
		Database databaseWithSchemaMeta = new Database(dbmsMeta, "SchemaMetaIT", catalog, schema);
		new DatabaseServiceFactory(sqlService).simple(config).gatherSchemaDetails(databaseWithSchemaMeta, schemaMeta,
				progressListener);
	...
	public void addColumn() throws Exception {
		...
		SchemaMeta schemaMeta = new SchemaMeta("src/test/resources/integrationTesting/schemaMetaIT/input/addColumn.xml",
				"SchemaMetaIT", schema, false);
		Database databaseWithSchemaMeta = new Database(dbmsMeta, "SchemaMetaIT", catalog, schema);
		new DatabaseServiceFactory(sqlService).simple(config).gatherSchemaDetails(databaseWithSchemaMeta, schemaMeta,
				progressListener);
	...
	public void disableImpliedOnAgentAccountId() throws Exception {
		...
		SchemaMeta schemaMeta = new SchemaMeta(
				"src/test/resources/integrationTesting/schemaMetaIT/input/disableImpliedOnAgent.xml", "SchemaMetaIT",
				schema, false);
		Database databaseWithSchemaMeta = new Database(dbmsMeta, "SchemaMetaIT", catalog, schema);
		new DatabaseServiceFactory(sqlService).simple(config).gatherSchemaDetails(databaseWithSchemaMeta, schemaMeta,
				progressListener);
	...
	public void addFKInsteadOfImplied() throws Exception {
		...
		SchemaMeta schemaMeta = new SchemaMeta(
				"src/test/resources/integrationTesting/schemaMetaIT/input/addFKInsteadOfImplied.xml", "SchemaMetaIT",
				schema, false);
		Database databaseWithSchemaMeta = new Database(dbmsMeta, "SchemaMetaIT", catalog, schema);
		new DatabaseServiceFactory(sqlService).simple(config).gatherSchemaDetails(databaseWithSchemaMeta, schemaMeta,
				progressListener);
	...
	public void disableDiagramAssociations() throws Exception {
		...
		SchemaMeta schemaMeta = new SchemaMeta(
				"src/test/resources/integrationTesting/schemaMetaIT/input/disableDiagramAssociations.xml",
				"SchemaMetaIT", schema, false);
		Database databaseWithSchemaMeta = new Database(dbmsMeta, "SchemaMetaIT", catalog, schema);
		new DatabaseServiceFactory(sqlService).simple(config).gatherSchemaDetails(databaseWithSchemaMeta, schemaMeta,
				progressListener);

		DotFormatter dotFormatter = new DotFormatter(new SimpleDotConfig(
				new DefaultFontConfig(config.getFont(), config.getFontSize()), config.isRankDirBugEnabled(), false,
				config.isNumRowsEnabled(), config.isOneOfMultipleSchemas()));

		StringWriter withoutSchemaMetaOutput = new StringWriter();
		try (PrintWriter printWriter = new PrintWriter(withoutSchemaMetaOutput)) {
			dotFormatter.writeTableAllRelationships(database.getTablesMap().get("COMPANY"), false, new LongAdder(),
					printWriter);
		}
		StringWriter withSchemaMetaOutput = new StringWriter();
		try (PrintWriter printWriter = new PrintWriter(withSchemaMetaOutput)) {
			dotFormatter.writeTableAllRelationships(databaseWithSchemaMeta.getTablesMap().get("COMPANY"), false,
					new LongAdder(), printWriter);
		}
